...
#define KEY_AMOUNT	4
	...
	struct timer_list mytimer;	// 动态延时函数
};
...
static irqreturn_t mykey_irq_handler(int irq, void *dev_id){
	...
	keys_irq.mytimer.data=(unsigned long)ki;
	
	mod_timer(&keys_irq.mytimer,jiffies+msecs_to_jiffies(20));
...
static int __init mykey_init(void){
	...
	//超时处理函数
	keys_irq.mytimer.function=mytimer_func;
	
	//初始化动态定时器
	init_timer(&keys_irq.mytimer);
